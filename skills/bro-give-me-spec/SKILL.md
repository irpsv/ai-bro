...
- После передачи спецификации (`spec.md` в memory-bank или файл через `CreatePlan`) **ОБЯЗАТЕЛЬНО** проведи [ревью спецификации](./references/spec-review.md), если человек явно не попросил обойтись без ревью.
...
- **ЗАПРЕЩЕНО** при доступном `CreatePlan` (или аналоге) спрашивать путь memory-bank или писать файл в memory-bank самому вместо вызова тула.
...
При выборе этого workflow **НЕ СПРАШИВАЙ** путь memory-bank.
