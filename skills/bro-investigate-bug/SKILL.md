---
name: bro-investigate-bug
description: Оркестрирует только READONLY расследование бага/регрессии/нестабильного поведения до root cause до планирования. Use when the user asks to investigate, reproduce, find root cause, explain why something breaks, triage a regression, or debug before a plan — without implementing fixes or writing an implementation plan.
---
...
Это скилл-оркестратор: он **только** управляет расследованием через субагента /investigator. Сам оркестратор **не** пишет код, **не** правит файлы, **не** составляет план исправления и **не** подменяет следующий этап планирования или этап реализации и проверки.
...
1. **Делегируй** основную работу субагенту /investigator, чтобы не раздувать основной контекст длинными обходами.
2. **НЕ СОБИРАЙ** и **НЕ АНАЛИЗИРУЙ** сам проблему, делегируй работу субагенту /investigator.
...
5. Оркестратор **собирает** ответ субагента и при необходимости задаёт **один узкий** follow-up второму запуску /investigator, если не хватает одной проверки — без превращения в бесконечный цикл.
...
## Субагент /investigator

Ниже — **контракт** субагента: его достаточно, чтобы запускать субагента /investigator и проверять ответ, без внешних спецификаций.
...
### Ограничения READONLY (для /investigator)
...
- просьбы «сразу сделай план» или «сразу поправь» — это вне роли /investigator.

### Формат ответа /investigator
...
```markdown
Ты investigator субагент для READONLY расследования бага или регрессии.
...
После ответа /investigator оркестратор выдаёт пользователю краткий итог:
...
Запуск расследования — только через /investigator.

1. Уточни у пользователя симптом, окружение и критерий «готово», если без этого нельзя начать безопасно.
2. Запусти /investigator с сжатым контекстом и явными READONLY-ограничениями.
3. Проверь, что ответ содержит проверенные факты, отброшенные гипотезы и явный статус уверенности.
4. Если один узкий пробел мешает статусу (например не хватает одного лог-фрагмента), запроси у пользователя **или** сделай **один** дополнительный узкий прогон /investigator.
