...
2. Передавай в промпт субагента **минимально достаточный** контекст: симптом, версия/окружение, шаги воспроизведения, что уже известно, ограничения READONLY и границы расследования.
3. **Доказательства важнее догадок**: требуй ссылок на конкретные файлы/строки/логи/симптомы; гипотезы без проверки не смешивай с выводами.
4. Оркестратор **собирает** ответ субагента и при необходимости задаёт **один узкий** follow-up второму запуску /investigator, если не хватает одной проверки — без превращения в бесконечный цикл.
5. Итог расследования должен быть **готов для планирования**: ясно, что доказано, что отброшено, что неизвестно, какой следующий человеческий или планирующий шаг уместен.
6. Не смешивай это расследование с последующей реализацией по коду или «сделай фикс» в одном прогоне — сначала расследование, потом отдельно следующий этап планирования или последующая реализация по желанию пользователя.

## Субагент /investigator

Ниже — **контракт** субагента: его достаточно, чтобы запускать /investigator и проверять ответ, без внешних спецификаций.
